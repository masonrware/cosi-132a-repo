...
def search(index: str, query: Query, n: int) -> None:
...
def rescore_search(index: str, query: Query, rescore_query: Query, n: int) -> None:
    s = Search(using="default", index=index).query(query)[
        :n
    ]  # initialize a query and return top five results
    s = s.extra(
...
if __name__ == "__main__":
    ...
    rescore_search(
        "wapo_docs_50k", q_basic, q_vector
    )  # rescore based on vector cosine similarity
