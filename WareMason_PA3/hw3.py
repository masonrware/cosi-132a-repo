...
class flaskApp:
    """This is the class for the flask app that serves as the backend
    ...
    def results():
        ...
        if len(postings_list) != 0:
            ...
            if len(res) != 0:
                pages[dict_ind] = res
            return render_template("results.html", response=pages[1], query=query_text, stop_words = stop_words, unknown_words = unknown_words, 
                                    PAGE_NUM=PAGE_NUM, TOTAL_PAGES=TOTAL_PAGES)  # add variables as you wish
        else:
    ...
    def prev_page(page_id: int) -> str:
        ...
        PAGE_NUM = page_id
        return render_template("results.html", response=pages[PAGE_NUM], query=query_text, stop_words = stop_words, unknown_words = unknown_words,
